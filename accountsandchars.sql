...
DROP PROCEDURE IF EXISTS call_clean_database;
DROP PROCEDURE IF EXISTS createCustomCharOnlineLockTable;
...
BEGIN
    ...
    CASE
        ...
        WHEN (class = 20) THEN
            ...
            SET max_sp = ROUND((RAND() * (2454-1435))+1435);
                 ELSE
            -- If class does not match any case, set default values
            SET str = 1;
            SET agi = 1;
            SET vit = 1;
            SET `int` = 1;
...
CREATE PROCEDURE createBotChars(IN account_pattern VARCHAR(50))
BEGIN
    DECLARE i INT DEFAULT 0;
    ...
    CREATE TEMPORARY TABLE temp_save_maps (name VARCHAR(30), x INT, y INT, weight INT);
    INSERT INTO temp_save_maps (name, x, y, weight) VALUES
        ('aldebaran', 142, 109, 5),
        ('payon', 161, 57, 10),
        ('geffen', 119, 40, 30),
        ('alberta', 118, 57, 10),
    ...
    -- Create temporary table for last map names
    DROP TEMPORARY TABLE IF EXISTS temp_last_maps;
    CREATE TEMPORARY TABLE temp_last_maps (name VARCHAR(30), weight INT);
    INSERT INTO temp_last_maps (name, weight) VALUES
        ('aldebaran', 10),
        ('yuno', 5),
        ('alberta', 10),
        ('geffen', 10),
    ...
    charPopulateLoop: LOOP
        ...
        -- Select a random save map considering weights
        SET @total_weight = (SELECT SUM(weight) FROM temp_save_maps);
        SET @rand_value = FLOOR(RAND() * @total_weight) + 1;

        SET @cumulative_weight = 0;
        SELECT name, x, y INTO @save_map_name, @save_x, @save_y
        FROM temp_save_maps
        WHERE (@cumulative_weight := @cumulative_weight + weight) >= @rand_value
        LIMIT 1;

        -- Select a random last map considering weights
        SET @total_weight = (SELECT SUM(weight) FROM temp_last_maps);
        SET @rand_value = FLOOR(RAND() * @total_weight) + 1;

        SET @cumulative_weight = 0;
        SELECT name INTO @last_map_name
        FROM temp_last_maps
        WHERE (@cumulative_weight := @cumulative_weight + weight) >= @rand_value
        LIMIT 1;

        CALL getRandomLocation(@last_map_name, @x, @y);
...
CREATE PROCEDURE createItemsForChars(IN account_pattern VARCHAR(50))
BEGIN
    ...
    DECLARE cur CURSOR FOR
        SELECT char_id, class, sex
        FROM `char`
        WHERE account_id IN (SELECT account_id FROM `login` WHERE userid LIKE CONCAT(account_pattern, '%'));
...
CREATE PROCEDURE createSkillsForChars(IN account_pattern VARCHAR(50))
BEGIN
        DECLARE cur_char_id INT(11) UNSIGNED;
        DECLARE cur_sex ENUM('M','F','S') DEFAULT 'F';
    DECLARE cur_class SMALLINT(6) UNSIGNED;
...
BEGIN
    -- Get account IDs for bot accounts
    CREATE TEMPORARY TABLE temp_bot_accounts AS
    SELECT account_id
    FROM `login`
    WHERE userid LIKE 'botijo%';

    -- Delete inventory items for bot characters
    ...
    WHERE char_id IN (
        ...
        WHERE account_id IN (
            SELECT account_id
            FROM temp_bot_accounts
        )
    );

    -- Delete skills for bot characters
    DELETE FROM skill
    WHERE char_id IN (
        SELECT char_id
        FROM `char`
...
BEGIN
    CREATE TABLE IF NOT EXISTS custom_char_online_lock (
